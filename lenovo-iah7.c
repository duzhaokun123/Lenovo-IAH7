...
// IO ports used to communicate with embedded controller
// Start of used ports
...
static ssize_t ecram_portio_init(struct ecram_portio* ec_portio) {
    if (!request_region(ECRAM_PORTIO_START_PORT, ECRAM_PORTIO_PORTS_SIZE,
                        ECRAM_PORTIO_NAME)) {
        pr_err("Cannot init ecram_portio the %x ports starting at %x\n",
 ...
 *
 * Returns status for a common signature shared with
 * other EC RAM access methods.
 */
...
static u8 ec_read(struct ecram_portio* ec_portio, u16 ecram_offset) {
    ...
    if (err)
        pr_err("Error reading EC RAM at 0x%x.\n", ecram_offset);
    return value;
}
...
static void ec_write(struct ecram_portio* ec_portio, u16 ecram_offset, u8 value) {
    ...
    if (err)
        pr_err("Error writing EC RAM to 0x%x.\n", ecram_offset);
}
...
static int iah7_hwmon_init(struct iah7_private* priv) {
...
static void iah7_hwmon_exit(struct iah7_private* priv) {
    if (priv->hwmon_dev) {
...
static int iah7_device_probe(struct platform_device* pdev) {
    ...
    dev_info(&pdev->dev, "lenovo_iah7 platform device probe\n");
    dev_info(&pdev->dev,
    ...
    do_load = force || dmi_sys != NULL;

    if (!do_load) {
        dev_info(&pdev->dev, "lenovo_iah7 not matched\n");
        err = -ENODEV;
        goto err_cleanup_shared;
    }
    ...
    dev_info(&pdev->dev, "loaded for device: %s\n", dmi_sys->ident);
...
static void iah7_device_remove(struct platform_device* pdev) {
    ...
    iah7_hwmon_exit(priv);
    ecram_portio_exit(&priv->ecram_portio);
    iah7_shared_exit(priv);
    dev_info(&pdev->dev, "lenovo_iah7 unloaded\n");
}

static struct platform_driver iah7_driver = {
    .probe = iah7_device_probe,
...
static void __exit iah7_exit(void) {
    platform_device_unregister(iah7_pdev);
    platform_driver_unregister(&iah7_driver);
